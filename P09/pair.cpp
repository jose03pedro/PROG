...
void veracidade(int index, std::vector<Pair<std::string, int>>& vetor) {
    sort(vetor.begin(), vetor.end(), [index](Pair<std::string, int> par_A, Pair<std::string, int> par_B) -> bool {
        if(index == 0) return par_A.get_first() < par_B.get_first();
        return par_A.get_second() < par_B.get_second();
    });
}

void sort_by_first(std::vector<Pair<std::string, int>>& vetor){veracidade(0, vetor);}

void sort_by_second(std::vector<Pair<std::string, int>>& vetor){veracidade(1, vetor);}
